...
from breez_sdk_spark import PaymentStatus as SparkPaymentStatus
...
class SparkWallet(LightningBackend):
    ...
    def __init__(self, unit: Unit = Unit.sat, **kwargs):
        ...
        network_name = settings.mint_spark_network.lower()
        self.network = Network.MAINNET if network_name == "mainnet" else Network.TESTNET
        self.storage_dir = settings.mint_spark_storage_dir
        self.connection_timeout = settings.mint_spark_connection_timeout
        self.max_retry_attempts = settings.mint_spark_retry_attempts
    ...
    async def pay_invoice(
        ...
        try:
            ...
            if fee_sats is not None and fee_sats * 1000 > fee_limit_msat:
                logger.warning(
                    "Spark payment fee exceeded limit after settlement: "
                    f"{fee_sats} sats > {fee_limit_msat / 1000} sats"
                )
        ...
        while True:
            try:
                ...
                if status.settled:
                    ...
                    logger.debug(
                        f"Spark stream saw unsettled payment {checking_id[:20]}, "
                        "skipping"
                    )
    ...
    async def _connect_sdk_with_retry(self) -> BreezSdk:
        ...
        while attempt < self.max_retry_attempts:
            try:
                ...
                delay = 2**attempt
                logger.warning(
                    f"Spark SDK connect attempt {attempt + 1} failed: {exc}; "
